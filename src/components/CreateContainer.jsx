	...
	const uploadImage = (e) => {
		...
		uploadTask.on(
			...
			(error) => {
				console.log(error);
				setFields(true);
				setMsg("Error uploading file : Try again 🙇‍♀️");
	...
	const saveDetails = () => {
		...
		try {
			if (!title || !calories || !price || !category || !imageAsset) {
				setFields(true);
				setMsg("Required fields missing : Try again 🙇‍♀️");
				setAlertStatus("danger");
				setTimeout(() => {
					setFields(false);
					setIsLoading(false);
			...
			console.log(error);
			setFields(true);
			setMsg("Error saving details : Try again 🙇‍♀️");
			setAlertStatus("danger");
			setTimeout(() => {
				setFields(false);
				setIsLoading(false);
		...
		<div className="w-full min-h-screen flex items-center justify-center">
			...
			gap-4 justify-center">
				...
				<div className="w-full py-2 border-b border-gray-300 flex items-center gap-2">
					<MdFastfood className="text-xl text-gray700" />
					<input
						type="text"
						required
						value={title}
						...
						className="w-full h-full text-lg bg-transparent outline-none border-0
				...
				<div className="w-full flex flex-col md:flex-row items-center gap-3">
					<div className="w-full py-2 border-b border-gray-300 flex items-center gap-2">
						...
						<input
							...
							className="w-full h-full text-lg bg-transparent text-textColor outline-none 
